...
x2 = np.linspace(-2*lb0,2*lb0,150)
y2 = 0
z2 = x2  
...
E_r = np.zeros_like(X2, dtype = complex)
E_theta = np.zeros_like(X2, dtype = complex)
E_phi = np.zeros_like(X2, dtype = complex)
...
for i in range(1,15):
    for j in range(-i,i+1):
        ...
        Mr, Mt, Mp = mp.M_nm(k0, r2, theta2, phi2, i, j)
        Nr, Nt, Np = mp.N_nm(k0, r2, theta2, phi2, i, j)
        # Calculo aenm y anmn
...
plt.clf()
plt.pcolormesh(X2,Z2,E_abs2, cmap ="hot_r")     
